...
<!-- NB: Je pense qu'il vaut mieux supprimer cette diapo sur les BDD. D'une, ça va embrouiller les gens alors qu'on est censés être dans les rappels. De deux, ça me semble un peu obsolète par rapport au module parquet/DuckDB -->
...
<!-- Je déplace à la fin, car ce n'est plus un rappel et il faut avoir parler de summarise avant d'aborder reframe. Voir si l'on crée un module Tidyverse avancé avec qq fonctions avancées, across et purr -->
...
-   Comme la fonction `summarise()`, la fonction `reframe()` **accepte
    l'argument `.by`** pour éviter d'écrire un group_by() et **est
    compatible avec la fonction across()** (voir plus loin).
