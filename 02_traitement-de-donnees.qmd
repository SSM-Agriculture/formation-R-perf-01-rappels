...
-  [Objectif de *dplyr* : rassembler dans un seul package les outils de manipulation de données les plus importants pour l'analyse des données]{.Terre_battue2}
...
-   Deux principes pour les packages *dplyr* et *tidyr* :
    ...
    -   **Usage de fonctions « verbe »** toutes construites sur le même
        ...
        guillemets
    -   **Usage de l'opérateur pipe** (%>% ou |>).
        Avec RStudio, le raccourci clavier pour cet opérateur est : **Ctrl +
...
## Création ou modification de variables dans une table (1/3)
...
de créer plusieurs variables à la fois :
    ...
    -   `table ← table %>% mutate(new_var2 = fonction(var1))` → création
        à partir d'une formule

    -   `table ← table %>% mutate(new_var3 = var1 + var2)` → création à
        partir d'opérations arithmétiques

    -   `table ← table %>% mutate(new_var4 = vecteur1)` → création à
        partir de variables externes

Pour modifier une variable, on affecte la nouvelle valeur à une variable
existante

## Création ou modification de variables dans une table (2/3)

## Création ou modification de variables dans une table (3/3)
...
→ Voir également l'argument [.groups](https://dplyr.tidyverse.org/reference/summarise.html) de la fonction summarise() qui permet de gérer les niveaux de regroupement d'une table. Voir [cet exemple](https://gist.github.com/ddotta/58b9e3d41518576958a7b0e4924da57e).
...
-   `replace_na()` : remplacer toutes les valeurs manquantes NA par une valeur donnée
...
ou exporter quasiment tout type de fichier (.csv, .xlsx, .ods, .rds, .sas7bdat, .parquet, etc.)
...
types de fichier, (cf. [aide de rio](https://gesistsa.github.io/rio/) pour les connaître et spécifier
