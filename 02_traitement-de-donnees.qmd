...
-   Paramètres utiles pour aller plus loin :
    ...
    -   .keep : pour choisir si on garde les variables qui servent à
        construire une nouvelle variable (pour éviter un `select(-var1,-var2)` après)

    `table ← table %>% mutate(new_var1 = var1+var2, .after=ident,.keep="unused")`

Dans cet exemple, la nouvelle variable `new_var1` est placée après `ident`, et `var1` et `var2` sont supprimés
