...
-   Indexation **par position** : \[no de ligne, no de colonne\]
...
mon_df$letters[1:4]  # idem que ci-dessus mais comme c'est un vecteur je peux l'indexer aussi (4 premières valeurs)
mon_df["letters"] # attention si pas de virgule, renvoie un data.frame d'une colonne
...
mon_df[,1] # un vecteur avec la 1ère colonne,
mon_df$letters[1:4]  # idem que ci-dessus mais comme c'est un vecteur je peux l'indexer aussi (4 premières valeurs)
mon_df["letters"] # un data.frame avec uniquement les lignes 1 et 3 de la colonne `letters`
mon_df[["letters"]]
```
...
-   Avec **R de base**, on utilise l'opérateur d'affectation :
    ...
    -   `table["new_var"] <- valeur_constante` → création d'une
        constante
